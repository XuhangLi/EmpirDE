...
#' @param mu.fix.value Undocumented parameter by the original authors of AdaReg. Skip documentation here.
#' @param var.fix.value Undocumented parameter by the original authors of AdaReg. Skip documentation here.
#' @return Estimated coef and variance, and fitting info for the residuals.
#' @note Code edited by Xuhang Li to avoid global functions and package building errors.

AdaReg = function(X, y, gam.seq = seq(0, 3, by=.1), var.gp.id=NULL, mu.fix.value=NULL, var.fix.value=NULL, tol=10^(-4), step=50) {

  # initialization
  fit = stats::lm(y ~ X - 1)
  beta.0 = stats::coef(fit)
  res.0 = y - X %*% beta.0
  ...
  if (length(var.gp.nm) == 1) {
    var.sig.gp.0 = stats::var(res.0)
  } else {
    for (k in 1:length(var.gp.nm)) {
      var.sig.gp.0[k] = stats::var(res.0[var.gp.id == var.gp.nm[k]])
    }
  }
  #var.sig.0 = stats::var(res.0)
...
adapt.gam.rob.fit.fn = function (x.00, gam.seq, step=50, mu.fix=NULL, var.fix=NULL, bin.num=NULL) {
  ...
  for (i in 1:length(gam.seq)) {
    ...
    x.var = ifelse(is.null(var.fix), stats::var(x.0), var.fix)
  ...
  if (length(gam.comp)  == 0 ) {
    ...
    w.nu = stats::dnorm(x.00, est.hat["mu0.hat"], est.hat["sd0.hat"])^est.hat["gam.sel"]
    w.nu[is.na(x.00)] = NA
...
efdr.0.fn = function (x, mu.hat, var.hat, gam, bin.num=NULL) {
  x = x[!is.na(x)]
  den.fit = stats::dnorm(x, mu.hat, sqrt(var.hat))
  ...
  p0.hat.bin[1] = stats::pnorm(bin.bk[2], mu.hat, sqrt(var.hat))
  p0.hat.bin[length(bin.bk)-1] = 1 - stats::pnorm(bin.bk[length(bin.bk)-1], mu.hat, sqrt(var.hat))
  for ( j in 3:(length(bin.bk)-1)) {
    p0.hat.bin[j-1] = stats::pnorm(bin.bk[j], mu.hat, sqrt(var.hat)) - stats::pnorm(bin.bk[j-1], mu.hat, sqrt(var.hat))
  }
...
est.fn = function(x, mu.0, var.0, gam,  tol=10^(-4), step=step, fix.mu=FALSE, fix.var=FALSE) {
  x = x[!is.na(x)]
  n = length(x)
  dum = stats::dnorm(x, mu.0, sqrt(var.0))^gam
  w.0 = dum/sum(dum)
...
bk.cnt.fn = function (x, bin.num=NULL) {
  ...
  h = graphics::hist(x, breaks=bk.num, plot=FALSE)
  # h$counts
  # h$breaks
